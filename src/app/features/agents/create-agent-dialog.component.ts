    ...
    >
      <div class="cf-form">
        <div class="cf-field">
          ...
          <input pInputText pSize="small" [(ngModel)]="name" placeholder="Mon agent" (keyup.enter)="submit()" autofocus />
        </div>
        <div class="cf-field">
          <label class="cf-label">Description <span class="cf-hint">(optionnel)</span></label>
          <textarea pTextarea pSize="small" [(ngModel)]="description" placeholder="Décrivez le rôle de cet agent…" rows="3" style="resize: none; width: 100%;"></textarea>
        </div>
      </div>
